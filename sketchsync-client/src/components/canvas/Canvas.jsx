...
import {
  drawPen, drawEraser, drawLine, drawRect, drawCircle, drawText,
  clearCanvas, commitPreviewToMain, renderSingleStroke, drawImageOnCanvas
} from '../../utils/canvasUtils';
import { v4 as uuidv4 } from 'uuid';

const Canvas = forwardRef(function Canvas({ socket, sessionId, onStickyAdd }, ref) {
  ...
  const resizeCanvases = useCallback(() => {
    ...
    if (main.width > 0 && main.height > 0) {
      try {
        imageData = main.getContext('2d').getImageData(0, 0, main.width, main.height);
      } catch (e) {}
    }
  ...
  useEffect(() => {
    ...
    socket.setCallbacks({
      ...
      onShape: ({ strokeId, type, startX, startY, endX, endY, color: c, brushSize: bs, undone = false }) => {
        const main = mainCanvasRef.current;
        if (!main) return;
        const ctx = main.getContext('2d');
        const data = { startX, startY, endX, endY, color: c, brushSize: bs, opacity: 1 };
        if (!undone) {
        ...
        localStrokes.current.push({ strokeId, type, startX, startY, endX, endY, color: c, brushSize: bs, undone });
      },
      ...
      onStateSync: (base64) => {
        ...
        img.onload = () => {
          ...
          drawImageOnCanvas(mainCanvasRef.current, base64);
        };
  ...
  const redrawPreview = useCallback(() => {
    ...
    if (isDrawing.current && currentPoints.current.length > 1) {
      ...
      else if (activeTool === TOOLS.ERASER) drawEraser(ctx, currentPoints.current, eraserSize);
  ...
  const onPointerMove = useCallback((e) => {
    ...
    socket?.emit(SOCKET_EVENTS.CURSOR_MOVE, { x: pos.x, y: pos.y, roomId: room?._id });
  ...
  }, [activeTool, color, brushSize, eraserSize, opacity, socket, sessionId, pushUndo]);
  ...
  const handleTextSubmit = useCallback(() => {
    ...
    setTextInput(null);
    setTextValue('');
  }, [textInput, textValue, color, socket, sessionId, pushUndo]);
  ...
  const getCursor = () => {
    ...
    const svgString = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}"><circle cx="${size/2}" cy="${size/2}" r="${size/2 - 1}" fill="none" stroke="#${strokeHex}" stroke-width="2" opacity="0.6"/></svg>`;
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-hidden" style={{ cursor: getCursor() }}>
      ...
      <canvas
        ...
        className="absolute inset-0"
        style={{ zIndex: 2, touchAction: 'none', mixBlendMode: 'multiply' }}
