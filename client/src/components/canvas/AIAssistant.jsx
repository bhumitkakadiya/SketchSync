...
export default function AIAssistant({ socket, sessionId }) {
  ...
  const handleSend = async (e, forcedMessage = null) => {
    ...
    try {
      // Capture the canvas if we need to describe it
      let context = null;
      if (userMessage.toLowerCase().includes('describe') || userMessage.toLowerCase().includes('what is on')) {
        const canvas = document.querySelector('canvas');
        if (canvas) {
          context = { imageBase64: canvas.toDataURL('image/png').split(',')[1] };
        }
      }

      const { data } = await aiService.chat(userMessage, context);
      ...
      switch (action.type) {
        ...
        case 'DRAW_TEXT':
          ...
          socket.emit(SOCKET_EVENTS.CANVAS_SHAPE, {
            ...
            startX: action.startX || 400,
            startY: action.startY || 400,
            endX: action.endX || 400,
            endY: action.endY || 400,
            text: action.text || '',
  ...
  return (
    <div className="absolute right-4 md:right-8 top-24 bottom-24 w-80 glass-dark rounded-2xl shadow-2xl flex flex-col z-20 animate-slide-up overflow-hidden">
      {/* Header */}
      <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--bg-border)' }}>
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>AI Assistant</h3>
        </div>
        <button onClick={toggleAI} className="btn-ghost p-1">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
        {messages.map(msg => (
          <div key={msg.id} className={`max-w-[85%] rounded-2xl px-4 py-2 ${msg.role === 'user' ? 'bg-brand-600 text-white self-end rounded-br-none' : 'self-start rounded-bl-none border'}`} style={{ background: msg.role === 'user' ? undefined : 'var(--bg-input)', borderColor: 'var(--bg-border)', color: msg.role === 'user' ? undefined : 'var(--text-primary)' }}>
            <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
          </div>
        ))}
        {isThinking && (
          <div className="max-w-[85%] rounded-2xl px-4 py-3 self-start rounded-bl-none border flex items-center gap-1" style={{ background: 'var(--bg-input)', borderColor: 'var(--bg-border)' }}>
      ...
      {/* Input */}
      <div className="p-3 border-t" style={{ borderColor: 'var(--bg-border)', background: 'var(--bg-card)' }}>
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
          {quickPrompts.map((prompt) => (
            <button
              key={prompt}
              onClick={() => handleSend(null, prompt)}
              className="text-[10px] whitespace-nowrap px-2 py-1 rounded-md transition-colors border"
              style={{ background: 'var(--bg-input)', borderColor: 'var(--bg-border)', color: 'var(--text-secondary)' }}
            >
              {prompt}
            </button>
          ))}
        ...
        <form onSubmit={(e) => handleSend(e)} className="relative">
          <input
            ...
            className="w-full rounded-xl pl-4 pr-10 py-2.5 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all shadow-sm border"
            style={{ background: 'var(--bg-input)', borderColor: 'var(--bg-border)', color: 'var(--text-primary)' }}
          />
          <button 
            type="submit"
