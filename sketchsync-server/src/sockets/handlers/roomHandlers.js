    ...
    try {
      ...
      if (room.activeSession) {
        ...
        strokes.forEach(stroke => {
          if (stroke.type === 'text') {
             socket.emit('CANVAS:TEXT', { strokeId: stroke.strokeId, userId: stroke.userId, x: stroke.data.startX, y: stroke.data.startY, text: stroke.data.text, fontSize: stroke.data.fontSize, color: stroke.data.color, undone: stroke.undone });
          } else if (['line', 'rect', 'circle'].includes(stroke.type)) {
             socket.emit('CANVAS:SHAPE', { strokeId: stroke.strokeId, userId: stroke.userId, type: stroke.type, startX: stroke.data.startX, startY: stroke.data.startY, endX: stroke.data.endX, endY: stroke.data.endY, color: stroke.data.color, brushSize: stroke.data.brushSize, undone: stroke.undone });
          } else if (stroke.type === 'clear') {
             socket.emit('CANVAS:CLEAR');
          } else if (stroke.data && stroke.data.points && stroke.data.points.length > 0) {
             const firstPoint = stroke.data.points[0];
             socket.emit('CANVAS:STROKE_START', { strokeId: stroke.strokeId, userId: stroke.userId, type: stroke.type, color: stroke.data.color, size: stroke.data.brushSize, x: firstPoint.x, y: firstPoint.y });
             socket.emit('CANVAS:STROKE_END', { strokeId: stroke.strokeId, userId: stroke.userId, finalPoints: stroke.data.points, undone: stroke.undone, type: stroke.type, color: stroke.data.color, brushSize: stroke.data.brushSize });
          }
        });
      }
